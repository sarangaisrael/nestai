CREATE OR REPLACE FUNCTION public.initialize_user_access(p_role access_role, p_therapist_code text DEFAULT NULL::text)
 RETURNS TABLE(user_id uuid, role access_role, payment_status access_payment_status, trial_ends_at timestamp with time zone, linked_therapist_id uuid, therapist_code text)
 LANGUAGE plpgsql
 SECURITY DEFINER
