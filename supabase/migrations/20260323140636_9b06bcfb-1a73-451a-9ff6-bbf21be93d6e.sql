...
BEGIN
  ...
  IF p_approve AND v_registration.user_id IS NOT NULL THEN
    ...
    -- Create user_access_statuses entry with therapist code
    v_generated_code := public.generate_unique_therapist_code();

    INSERT INTO public.user_access_statuses (user_id, role, payment_status, therapist_code)
    VALUES (v_registration.user_id, 'therapist'::public.access_role, 'active'::public.access_payment_status, v_generated_code)
    ON CONFLICT (user_id) DO UPDATE
    SET therapist_code = COALESCE(public.user_access_statuses.therapist_code, EXCLUDED.therapist_code),
