...
const AdminDashboard = () => {
  ...
  const loadAllData = async () => {
    setLoadingData(true);
    // Load users first so getUserEmail works correctly when accessRows arrive
    const usersResult = await loadUsersAndReturn();
  ...
  const loadUsersAndReturn = async (): Promise<AdminUser[]> => {
    try {
      ...
      setUsers(mapped);
      console.log('users loaded:', mapped);
      return mapped;
  ...
  const getUserEmail = (userId: string): string =>
    users.find((u) => u.user_id === userId)?.email ?? userId;

  const filteredAccessRows = accessRows.filter((row) => {
    if (!normalizedAccessSearch) return true;
    return [getUserEmail(row.user_id), row.role, row.payment_status, row.therapist_code ?? "", row.linked_therapist_id ?? ""]
                        ...
                        <TableRow key={row.user_id}>
                          <TableCell>
                            <div className="space-y-1">
                              ...
                              <p className="text-[11px] text-muted-foreground font-mono">{row.user_id}</p>
